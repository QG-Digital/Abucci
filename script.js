...
const lightboxStyles = `
    ...
    .lightbox-content { position: relative; max-width: 90%; max-height: 90%; display: flex; align-items: center; justify-content: center; animation: zoomIn 0.3s ease; }
    @keyframes zoomIn { from { transform: scale(0.8); opacity: 0; } to { transform: scale(1); opacity: 1; } }
    
    /* --- ESTA É A PARTE CORRIGIDA --- */
    .lightbox-content img { 
        max-width: 100%;          /* Garante que a imagem não ultrapasse a largura do seu contêiner */
        max-height: 90vh;         /* Garante que a imagem não ultrapasse 90% da altura da tela */
        width: auto;              /* Deixa o navegador calcular a largura proporcional */
        height: auto;             /* Deixa o navegador calcular a altura proporcional */
        border-radius: 10px; 
...
const observer = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
        if (entry.isIntersecting) {
            ...
            observer.unobserve(entry.target); // Opcional: para a animação rodar só uma vez
        }
    });
}, observerOptions);
